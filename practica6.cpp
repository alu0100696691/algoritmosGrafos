...
GRAFO::GRAFO (char nombreFichero[]) {		// Con CADENA + 1 podemos realmente almacenar el numero de caracteres que definimos en la constante, caracter nulo al final

	
	
	unsigned i, j;
	int c;
	float x,y,z;   //variables para calculo de las longitudes de las aristas
	x=y=z=0;
	m=0;
		...
		for(i=0;i<n;i++)
		  for(j=0;j<n;j++)
		  {
			...
			dummy.residuo = longitud.at(i);
                       	dummy.inv = LS[j].size();
			LS[i].push_back (dummy);				//leo fila (i,j)  si (1,2) -> guardo (0,1)
			dummy2.j = i;  
...
void GRAFO :: kruskal(){
  ...
  cout << "\n\nKRUSKAL: árbol de expansión mínimo del grafo.(longitudes)\n\n\n";
  for(int i = 0; i < 2; i++)
            for(int j = 0; j < 2; j++) 
	      cout << arbol[i][j] << "->";
...
vector<unsigned> pred, pospred;
   ...
   while (camino == true) {
	 ...
	 // actualizaciï¿½n de las capacidades residuales
